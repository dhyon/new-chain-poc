...
func TestShortBookQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetShortBookRequest{Id: msgs[0].Id},
			response: &types.QueryGetShortBookResponse{ShortBook: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetShortBookRequest{Id: msgs[1].Id},
			response: &types.QueryGetShortBookResponse{ShortBook: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetShortBookRequest{Id: uint64(len(msgs))},
